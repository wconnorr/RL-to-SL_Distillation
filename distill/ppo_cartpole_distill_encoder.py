...
from models.cartpole_models import Distiller, GTNGenerator, Critic, RLDataset, create_encoder_actor
...
def main():
  ...
  parser.add_argument("-e", "--encoder", help="if non-zero, the network will be split into an encoder and learner. Provide the position of the split.", type=int, default=-1)
...
#   randomize_inner_architecture = args.randomize_inner_architecture
  ...
  encoder, actor = create_encoder_actor(STATE_SPACE, ACTION_SPACE, args.encoder)
  ...
  for meta_epoch in range(num_meta_epochs):
    ...
    if use_consistent_setup:
      ...
      actor = create_encoder_actor(STATE_SPACE, ACTION_SPACE, args.encoder)[1].to(device)
