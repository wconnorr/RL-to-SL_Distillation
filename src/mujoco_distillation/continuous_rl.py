"""
TODO: Implement details from https://iclr-blog-track.github.io/2022/03/25/ppo-implementation-details/

TODO: Implement vecenv wrapper yourself
"""
...
from continuous_models import Actor, Critic, RLDataset
import vector_env
...
def main():
  ...
  parser.add_argument("-e", "--env", help="name of MuJoCo environment to learn on", choices=[name + '-' + MUJOCO_ENV_VERSION for name in MUJOCO_ENV_NAMES])
  ...
  env = vector_env.make_mujoco_vector_env(num_envs, args.env)
  ...
  last_state = torch.from_numpy(env.reset()[0]).float()
    ...
    for policy_epoch in range(policy_epochs):
      ...
      while not epoch_done:
        ...
        if gather_data:
          ...
          rollout_rewards, last_state, last_done = perform_rollout(policy_network, value_network, env, rollout, rollout_len, last_state, action_min, action_max)
  ...
  torch.save(policy_optimizer.state_dict(), os.path.join(results_path, str(epoch+1), 'policy_optim_sd.pt'))
  torch.save(value_optimizer.state_dict(), os.path.join(results_path, str(epoch+1), 'value_optim_sd.pt'))
...
def plot(path, rewards, policy_losses, value_losses, entropy_losses):
    ...
    with open(os.path.join(path, "rew.txt"), 'w') as f:
      f.write("${} \pm {}$".format(np.mean(rewards[-1000:]), np.std(rewards[-1000:])))
...
def calculate_losses(policy_network, value_network, transition, epsilon):
  ...
  # TODO: Make sure this is differentiable!!!
...
def perform_rollout(agent, critic, vec_env, rollout, rollout_len, state, action_min, action_max):
  with torch.no_grad():
    final_rewards = []
